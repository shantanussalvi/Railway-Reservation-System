...
conn = sqlite3.connect('railway.sqlite')
cursor = conn.cursor()
...
(trn_no TEXT UNIQUE, frm TEXT, to_ TEXT, trn_nme TEXT)''')
cursor.execute('''CREATE TABLE IF NOT EXISTS passengers 
(pid TEXT UNIQUE, trn_no TEXT, name TEXT, age TEXT, gender TEXT, phn_no TEXT, trn_nme TEXT, cls TEXT, frm TEXT, to_ TEXT, dte TEXT, tme TEXT)''')
...
for index, row in df.iterrows():
    ctr += 1
    cursor = conn.cursor()
    cursor.execute('''INSERT OR IGNORE INTO train(trn_no, trn_nme, frm, to_) values (?,?,?,?)''', (row["Train no."], row["Train name"], row["Starts"], row["Ends"]))
    if(ctr == 10):
        break
    conn.commit()
...
def bktktf(arrval): # Book Ticket Function For Database
    ...
    if ((mi!="") and (mi!=" ")):
            ...
            if(flag):
                ...
                cursor.execute("""insert into passengers(name,age,gender,phn_no,trn_nme,cls,frm,to_,dte,tme,pid,trn_no) values (?,?,?,?,?,?,?,?,?,?,?,?)""",arr)
...
def inuptrnque(fn,arrval):
    ...
    #update train set frm =? ,to_ =? ,dte =? ,tme =? ,trn_nme =? ,pantry =? where trn_no =? 

    if ((mi!="") and (mi!=" ")):

        if(fn=="Insert"):

            cursor.execute("""insert into train(frm,to_,trn_nme,trn_no) values (?,?,?,?)""",arr)
            ...
            flag = cursor.fetchall()
            if(flag):
                cursor.execute("""update train set frm =? ,to_ =?, trn_nme =? where trn_no =?""",arr)
...
def deltrnque(arrval):
    ...
    if ((mi!="") and (mi!=" ")):
        ...
        if(flag):
            cursor.execute("""delete from train where trn_no =?""",arr)
...
def cncltcktq(arrval):
    ...
    if ((mi!="") and (mi!=" ")):
        ...
        cursor.execute(""" select * from passengers where pid =? and trn_no = ?""", (arr[0], arr[1]))
        flag = cursor.fetchall()

        if(flag):
            cursor.execute("""delete from passengers where pid =?""",(arr[0],))
...
def trnadmn():
    ...
    trnadtree = ttk.Treeview(trnadmnwin, column=("c1", "c2", "c3", "c4"), show='headings')
    trnadtree.column("c1", width=150,anchor='c')
    trnadtree.heading("c1", text="Train no")

    trnadtree.column("c2", width=150,anchor='se')
    trnadtree.heading("c2", text="From")
    ...
    trnadtree.column("c3",width=150, anchor='se')
    trnadtree.heading("c3", text="To")

    trnadtree.column("c4",width=250, anchor='se')
    trnadtree.heading("c4", text="Train_Name")

    # trnadtree.column("c7",width=110, anchor='se')
    # trnadtree.heading("c7", text="Pantry")

    def select_record():
        selected = trnadtree.focus()
        global values
        values = trnadtree.item(selected, 'values')
    
    trnadtree.grid(row=1,column=0,columnspan=4,pady=3)


    selbtn = tk.Button(trnadmnwin,text="Select",command=select_record,height=2,width=16)
    upbtn = tk.Button(trnadmnwin,text="Update",command=lambda:[inuptrn("Update", values)],height=2,width=16)
    inbtn =  tk.Button(trnadmnwin,text="Insert",command=lambda:[inuptrn("Insert", values)],height=2,width=16)
    ...
    selbtn.grid(row=0,column=4,pady=5)

    print("Train - admin view")


def trnaval(): # Train Available-Meghesh
    
    trnavalwin = tk.Tk()
    trnavalwin.resizable(False,False)
    trnavalwin.title("Train Ticket Reservation")
    
    tree = ttk.Treeview(trnavalwin, column=("c1", "c2", "c3", "c4"), show='headings')
    tree.column("c1", width=110,anchor='c')
    tree.heading("c1", text="Train no")

    tree.column("c2", width=110,anchor='se')
    tree.heading("c2", text="From")

    tree.column("c3", width=110,anchor='se')
    tree.heading("c3", text="To")

    # tree.column("c4",width=110, anchor='se')
    # tree.heading("c4", text="Date")

    # tree.column("c5",width=110, anchor='se')
    # tree.heading("c5", text="Time")

    tree.column("c4",width=200, anchor='se')
    tree.heading("c4", text="Train Name")

    # tree.column("c7",width=110, anchor='se')
    # tree.heading("c7", text="Pantry")

    tree.grid(row=1,column=0,columnspan=4,pady=3)
    viewtrnaval(tree)
    ...
    # def selectItem():
    #     curItem = tree.focus()
    #     arr = [tree.item(curItem)["values"]]
    #     print(tree.item(curItem))
    #     return arr
        
        
    selectbtn = tk.Button(trnavalwin,text="Select", command=selectItem, height=2, width=8)#,tcktbookdmsg()
    selectbtn.grid(row=0,column=0,columnspan=2,padx=10,pady=10)
    submitbtn = tk.Button(trnavalwin,text="Book Ticket", command=lambda:[tcktres(values)], height=2, width=8)#,tcktbookdmsg()
    submitbtn.grid(row=0,column=1,columnspan=2,padx=10,pady=10)
...
def inuptrn(fn, values):

    try:
        print(values)
    except:
        warmsg("Please Select a record to update", "Train Ticket Reservation - Admin")
    ...
    Train_nol = Label(inupwin,text="Train no :").grid(row = 2,column=0,padx=5,pady=5)
    trn_no = Entry(inupwin,width=37)
    trn_no.insert(0,values[0])
    trn_no.grid(row = 2,column=1,padx=5,pady=5)

    Train_namel = Label(inupwin,text="Train Name :").grid(row = 3,column=0,padx=5,pady=5)
    trn_nme = Entry(inupwin,width=37, text=values[1])
    trn_nme.insert(3,values[3])
    trn_nme.grid(row = 3,column=1,padx=5,pady=5)
